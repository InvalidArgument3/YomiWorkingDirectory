...
func tick_before():
	...
	if queued_action:
		...
		if queued_action == "Continue":
			...
			elif (was_my_turn or (current_state().interruptible_on_opponent_turn and current_state().next_state_on_hold_on_opponent_turn)) and not feinting and current_state().next_state_on_hold:
